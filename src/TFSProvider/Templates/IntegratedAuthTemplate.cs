    ...
    public class TFSWorkItemStoreConnector
{
        ...
        {
            Uri tfsUri = new Uri(GetConfigValue("$ServiceInstance.TFSUri$"));

            //If you only provide the server's uri when connecting to TFS, the caller's Windows credentials will be used
            TfsConfigurationServer tfs = TfsConfigurationServerFactory.GetConfigurationServer(tfsUri);
        ...
        public static CatalogNode GetTeamProjectCollection(TfsConfigurationServer tfs)
        {
            ...
            return teamProjectCatalogs.FirstOrDefault(p => p.Resource.DisplayName == GetConfigValue("$ServiceInstance.TeamProjectCollectionName$"));
        }
        ...
        {
            return workItemStore.Projects[GetConfigValue("$ServiceInstance.TeamProjectName$")];
        }
